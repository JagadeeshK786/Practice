...
import com.game.http.ChannelWrapper.AsyncChannelSslWrapper;
import com.game.http.ChannelWrapper.AsyncChannelWrapper;
...
public final class HttpClient {
     ...
     * @param queryString   URL参数，可为null
    ...
    static public boolean postAsync(String ipAddress, int port, String host, String path,
	    QueryStringBuilder queryString, boolean isHttps, IHttpRequestCallback callback) {
	String body = null;
	if (queryString != null)
    ...
    // 读取到HTTP响应报文后
    static private void onRespond(Integer bytesTransferred, AsyncChannelWrapper channel, ByteBuffer readBuffer,
	...
	try {
		...
		fullBody = new ContentLengthBodyDecoder(bytesTransferred - beginOfBody).decode(channel, bodyBytes);
		...
		Utility.memcpy(bodyBytes, readBuffer.array(), beginOfBody, bytesTransferred - beginOfBody);
		fullBody = new ChunkedBodyDecoder().decode(channel, bodyBytes);
	    } else {
    ...
    static private AsyncChannelWrapper getChannelWrapper(boolean isHttps) {
	...
	if (isHttps)
	    return new AsyncChannelSslWrapper(channel);
	else
	...
	channel.connect(new InetSocketAddress(ipAddress, port), channel,
		new CompletionHandler<Void, AsyncChannelWrapper>() {
		    @Override
		    public void completed(Void arg0, AsyncChannelWrapper arg1) {
			// 连接成功后...
			...
			arg1.write(ByteBuffer.wrap(requestBytes), arg1,
				new CompletionHandler<Integer, AsyncChannelWrapper>() {
				    @Override
				    public void completed(Integer bytesTransferred, AsyncChannelWrapper attachment) {
					// 请求发送成功后...
					// 投递一个异步读请求
					...
					attachment.read(readBuffer, attachment,
						new CompletionHandler<Integer, AsyncChannelWrapper>() {
						    ...
						    public void completed(Integer bytesTransferred,
							    AsyncChannelWrapper attachment) {
							// 读取到响应后...
						    ...
						    @Override
						    public void failed(Throwable exc, AsyncChannelWrapper attachment) {
							// 读取HTTP响应失败
							try {
							    userCallbackExecutorService.execute(new Runnable() {
								@Override
		    ...
		    @Override
		    public void failed(Throwable arg0, AsyncChannelWrapper arg1) {
			// 连接到服务器失败
			userCallbackExecutorService.execute(new Runnable() {
			    @Override
			    public void run() {
